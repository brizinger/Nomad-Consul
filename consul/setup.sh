...
sed -i -- "s/__NODE_NAME__/$HOSTNAME/g" config.json
sed -i -- "s/__ENCRYPTION__/$ENCRYPT/g" config.json
sed -i -- "s/__PRIVATE_IP1__/$PRIVATE_IP1/g" config.json
sed -i -- "s/__PRIVATE_IP2__/$PRIVATE_IP2/g" config.json
sed -i -- "s/__PRIVATE_IP3__/$PRIVATE_IP3/g" config.json

if [ "$#" -eq 6 ];
then
    PRIVATE_IP4=$6
    sed -i -- "s/__PRIVATE_IP0__/$PRIVATE_IP4/g" config.json
fi
